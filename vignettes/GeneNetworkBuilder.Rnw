...
important parts in regulating gene expression\cite{Walhout2010}. 
...
To use GNB, users need to input a list of candidates genes bound by a given
TF and another list of genes/miRNAs with altered expression by knockdown of the
...
GNB provides two embeddedded regulatory networks. One is designed for \textit{Caenorhabditis elegans}
...
other one is designed for \textit{Homo sapiens} combining database FANTOM\cite{Ravasi2010},
miRGen\cite{Panagiotis2010} and microCosm Targets\cite{Sam2008}. FANTOM screened for physical
interactions among the majority of human/mouse DNA-binding TFs. The miRGen
is an integrated database of miRNA coding transcripts, their regulation by TFs and miRNA targets. 
...
\begin{enumerate}
	\item buildNetwork, build the network starting from the bound gene list and then 
iterating downstream regulatory events in GNB-embedded or user-defined regulatory network.
	\item filterNetwork, filter the network by differential expressed genes/miRNAs. 
	\item polishNetwork, generate the graphNEL object with a given color set and parameters 
can be passed to graph:graph.par.
\end{enumerate}
...
\subsection{A fully working example}
Here is a fully working example to use GNB to generate a simple regulatory network for 
...
\begin{figure}[htb]
\centering
\includegraphics{ce_example.eps}
\caption{Regulatory network of a fully working example}
...
\begin{figure}[htb]
\centering
\includegraphics{hs_sample_rcytoscape.eps}
